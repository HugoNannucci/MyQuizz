...
class CategorieController extends AbstractController
{
    ...
    //new categorie + for i < 10 create question + for j < 3 create reponse

    #[Route('/{id}', name: 'app_categorie_show', methods: ['GET'])]
    public function show(Categorie $categorie, EntityManagerInterface $entityManager): Response
    {
        $test = $entityManager->getRepository(Question::class)->findBy(array('categorie' => $categorie->getId()));
        $reponses = [];
        $reponseRepository = $entityManager->getRepository(Reponse::class);
       foreach ($test as $value) {
            $reponses[] = $reponseRepository->findBy(['question' => $value->getId()]);
       }
